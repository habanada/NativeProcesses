...
namespace ProcessServer
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //2F3FAD85ADDFB73AB2D0E30A552C6E942FA01DBF
            ...
            try
            {
                cert = new X509Certificate2(certPath, certPass, X509KeyStorageFlags.MachineKeySet);
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine("Check the password and ensure the file is a valid .pfx.");
            ...
            var server = new SecureTcpServer(port, certPath, certPass, token);
    ...
    public class ConsoleLogger : IEngineLogger
    {
        ...
        public void Log(LogLevel level, string message, Exception ex = null)
        {
            lock (_lock)
            {
                ...
                if (!message.Contains("Failed to open PID") && !message.Contains("Failed to get IoCounters") && !message.Contains("Failed to get CommandLine for PID"))
                {
