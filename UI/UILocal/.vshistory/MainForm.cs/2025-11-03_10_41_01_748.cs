...
namespace ProcessDemo
{
    public partial class MainForm : Form
    {
        ...
        private void LoadProcesses()
        {
            ...
            // KORREKTE ZUWEISUNG:
            // 1. Die BindingSource (der Filter) nutzt die Master-Liste als Quelle.
            _filteredBindingSource.DataSource = _allProcessItems;

            // 2. Das Grid nutzt die BindingSource (den Filter) als Quelle.
        ...
        private void ApplyFilter()
        {
            ...
            string filterText = edtFilter.Text.Trim();

            if (string.IsNullOrEmpty(filterText))
            {
                _filteredBindingSource.Filter = null;
            }
            else
            {
                string escapedFilter = filterText.Replace("'", "''").Replace("%", "[%]");

                try
                {
                    _filteredBindingSource.Filter = string.Format(
                        "Name LIKE '%{0}%' OR ExePath LIKE '%{0}%' OR CONVERT(Pid, 'System.String') LIKE '%{0}%'",
                        escapedFilter
                    );
                }
                catch (Exception)
        ...
        private void Service_ProcessAdded(FullProcessInfo info)
        {
            ...
            // Fügt zur Master-Liste hinzu. Das Grid wird automatisch aktualisiert.
        ...
        private void Service_ProcessRemoved(int pid)
        {
            ...
            {
                // Entfernt aus Master-Liste. Das Grid wird automatisch aktualisiert.
    ...
    public class ConsoleLogger : IEngineLogger
    {
        ...
        public void Log(LogLevel level, string message, Exception ex = null)
        {
            ...
            // KORREKTE FILTERUNG (gilt jetzt für ALLE Log-Level, inkl. Debug):
            ...
            if (lowerMessage.Contains("failed to open") ||
                ...
                lowerMessage.Contains("could not read process memory"))
            {
                return; // Ignorieren
            }

            // Falscher Parameter (PID 0) ebenfalls ignorieren
            if (lowerMessage.Contains("falscher parameter") && level == LogLevel.Debug)
            {
                return; // Ignorieren
            }
            ...
            if (_richTextBox.InvokeRequired)
                ...
                _richTextBox.Text += line + Environment.NewLine;
        }
    }
}