...
namespace NativeProcesses
{
    public class ManagedProcess : IDisposable
    {
        ...
        private string GetTokenIntegrityLevel(IntPtr tokenHandle)
        {
            ...
            {
                if (Marshal.GetLastWin32Error() != 122)
                    throw new Win32Exception(Marshal.GetLastWin32Error(), "GetTokenInformation(TokenIntegrityLevel) length failed.");
            }
        ...
        private bool GetTokenIsElevated(IntPtr tokenHandle)
        {
            ...
            {
                if (Marshal.GetLastWin32Error() != 122)
                    throw new Win32Exception(Marshal.GetLastWin32Error(), "GetTokenInformation(TokenElevation) length failed.");
            }
