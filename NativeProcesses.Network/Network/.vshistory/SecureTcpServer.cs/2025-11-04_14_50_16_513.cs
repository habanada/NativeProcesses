    ...
    public class SecureTcpServer
    {
        ...
        public async Task SendMessageAsync(SslStream ssl, string type, object data)
        {
            try
            {
                ...
                // ⬇ hier korrigieren: Strings und komplexe Objekte sauber trennen
                if (data is string s)
                    jsonData = s;
                else
                ...
                if (data is List<Core.FullProcessInfo>)
                {
                     //   var test = JsonConvert.DeserializeObject<List<Core.FullProcessInfo>>(jsonData);
                    }
